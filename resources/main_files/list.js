...
blocket("@apps.list").extend({
	...
	parse_numeric_range : function (from, to, suffix) {
		"use strict";
		from = (from ? from.replace(/[^\d\s,]/ig, '').trim() : '');
		to = (to ? to.replace(/[^\d\s,]/ig, '').trim() : '');
		if (from && to) {
	...
	enable_custom_stepping : function () {
		...
		blocket.$.on("click", function () {
			$.cookie_raw("sq", $(this).attr("rel"), {path : "/", domain : blocket.bconf.common.cookie_domain});
	...
	enable_custom_stepping_manual : function (custom_href) {
		...
		$.cookie_raw("sq", tmp_href, {path : "/", domain : blocket.bconf.common.cookie_domain});
		return true;
	},
	...
	replace_search_results : function (data, objthis) {
		...
		if (objthis.attr("data-param") !== "" && objthis.attr("data-val") !== "") {
			...
			// get current source
			var iframe_bostad = $("#verticals_banner_bostad").attr("src") !== undefined ? $("#verticals_banner_bostad").attr("src") : '';
			var iframe_jobb = $("#verticals_banner_jobb").attr("src") !== undefined ? $("#verticals_banner_jobb").attr("src") : '';
			...
			if ($.inArray(objthis.attr("data-param"), allowed_qs_bostad) !== -1) {
				if ($("#verticals_banner_bostad").length > 0) {
					...
					// refresh triple banner url
					$("#verticals_banner_bostad").attr("src", iframe_bostad);
				}
			}

			// JOBB
			if (allowed_qs_jobb[objthis.attr("data-param")] !== undefined) {
				if ($("#verticals_banner_jobb").length > 0) {
					...
					// refresh triple banner url
					$("#verticals_banner_jobb").attr("src", iframe_jobb);
				}
			}
...
blocket("@init.list.self").extend(function () {
	...
	if ($("#blocket.realestate").length === 0) {
